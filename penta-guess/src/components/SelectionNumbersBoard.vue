  ...
  <div class="border-solid  bg-stone-300 rounded-xl shadow border-2 m-5 gap-4  place-items-center">
    ...
    <div class="relative h-[17rem]">
      <!-- <button type="button" :class="this.selectedNumbers.length !== 5 ? 'cursor-not-allowed' : ''"
        class="rounded-md bg-cyan-600  text-sm font-semibold leading-6 text-white shadow-sm hover:bg-cyan-600 focus-visible:outline focus-visible:outline-2 disabled:opacity-30 absolute inset-x-0 top-0 h-16 w-20"
        @click="clearNumbers()" :disabled="this.selectedNumbers.length == 0">
        Clear All</button> -->

      <button type="button" :class="this.selectedNumbers.length !== 5 ? 'cursor-not-allowed' : ''"
        class="rounded-md bg-cyan-700  text-sm font-semibold leading-6 text-white shadow-sm hover:bg-cyan-600 focus-visible:outline focus-visible:outline-2 disabled:opacity-30 absolute inset-x-120 bottom-0 h-16 w-20"
